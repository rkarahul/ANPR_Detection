...
# Load ONNX models
# detection = nn.Detection(r'weights\detection.onnx')
# print("detection model loaded")
recognition = nn.Recognition(r'model_rec.onnx')
print("recognition model loaded")
# classification = nn.Classification(r'weights\classification.onnx')
# print("classification model loaded")
# Video path and model initialization
VIDEO_PATH_1 = r"plate.mp4"  # Change to live feed if needed
model = YOLO(r"best_openvino_model", task="detect")
...
def log_best_result(tracker_id, ocr_text, confidence):
    ...
    if tracker_id not in tracker_results:
        tracker_results[tracker_id] = deque(maxlen=10)
...
while True:
    try:
        ...
        for i in range(len(tracked_detections.xyxy)):
            ...
            # Use ONNX-based OCR for recognizing text
            ocr_results = recognition([plate_region])
            # print("ocr_results", ocr_results)
            # Process results: Assuming ocr_results has text and confidence as separate lists
            if ocr_results[0] is not None:
                recognized_texts = ocr_results[0]  # Extract recognized text
                print("recognized_texts", recognized_texts)
                confidence_scores = ocr_results[1]  # Extract confidence scores
                # Process OCR results for the current tracker ID
                for idx, text in enumerate(recognized_texts):
                    print("text", text)
                    if len(confidence_scores) > 0 and len(confidence_scores[idx]) > 0:
                        confidence_score = confidence_scores[idx]  # Get the corresponding confidence score
                        avg_confidence = sum(confidence_score) / len(confidence_score)  # Calculate average confidence
                    else:
                        avg_confidence = 0.0  # Set to 0 if no confidence scores available
                    processed_text = post_process_first_line(text)
                    # Log the best result for this tracker after checking the last 5 results
                    log_best_result(tracker_id, processed_text, avg_confidence)
